...
function PdfComponent() {
  ...
  const certificationList = useSelector(state => state.certificateList)
                  ...
                  certificationList.map((item,id)=>{
                    return(
                      <div className="d-flex justify-content-start py-1" key={id}>
                        ...
                        <div className="px-3">
                          <h4>{item.title}</h4>
                          <p className="m-0">{item.issuer} • {item.startMonth} {item.startYear} {`${item.isCompleted ? " - Present" : " - "+item.endMonth+" "+item.endYear }`}</p>
                          {/* <p className="m-0">{item.location}</p> */}
                          <p>{item.description}</p>
                        </div>
                      </div>
